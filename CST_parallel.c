...
int main(int argc, char **argv) {
	...
	// sync Values, Dif 
	uint64_t* globalValues;
	uint64_t* globalDifferenzes;
	if (rank == 0) {
		globalValues = (uint64_t*)malloc(numprocs * sizeof(uint64_t));
		MPI_Gather(
			&(best.value), 			// void* send_data, 
			1, 						// int send_count,
			MPI_UNSIGNED_LONG_LONG, // MPI_Datatype send_datatype,
			globalValues, 			// void* recv_data,
			1, 						// int recv_count,
			MPI_UNSIGNED_LONG_LONG,	// MPI_Datatype recv_datatype,
			0,						// int root,
			MPI_COMM_WORLD);		// MPI_Comm communicator)
		globalDifferenzes = (uint64_t*)malloc(numprocs * sizeof(uint64_t));
		MPI_Gather(
			&(best.differenz), 			// void* send_data, 
			1, 						// int send_count,
			MPI_UNSIGNED_LONG_LONG, // MPI_Datatype send_datatype,
			globalDifferenzes, 			// void* recv_data,
			1, 						// int recv_count,
			MPI_UNSIGNED_LONG_LONG,	// MPI_Datatype recv_datatype,
			0,						// int root,
			MPI_COMM_WORLD);		// MPI_Comm communicator)
	...
	if (rank == 0) {
		for (int p = 0; p < numprocs; p++) {
			if (best.differenz > globalDifferenzes[p]) {
				best.differenz = globalDifferenzes[p];
				best.value = globalValues[p];
			}
		}
	}
